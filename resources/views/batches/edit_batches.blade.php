    ...
    <div class="col-xl-12 col-lg-10">
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">Edit Batches</h6>
                <a href="{{route('batches.index')}}" class="btn btn-primary">Cancel</a>
            </div>
            <div class="card-body">
                <div class="col-8 mx-auto">
                    ...
                    <form action="{{route('batches.update',$batch)}}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="name" name="name" value="{{$batch->name}}" class="form-control @error('name') border-danger @enderror">
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('course') border-danger @enderror" name="course" id="course" >
                                ...
                                <option value="{{$item->id}}" @if ($batch->course_id == $item->id)selected @endif >{{$item->name}}</option>
                                @endforeach
                            </select>
                            @error('course')
                            <p class="text-danger">{{$message}}</p>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('start') border-danger @enderror" name="start" id="start" >
                                ...
                                @foreach($month as $m)
                                    <option value="{{$m}}" @if($batch->start == $m) selected @endif>{{$m}}</option>
                                @endforeach
                            </select>
                            @error('start')
                                <p class="text-danger">{{$message}}</p>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('end') border-danger @enderror" name="end" id="end" >
                                ...
                                @foreach($month as $m)
                                    <option value="{{$m}}"  @if($batch->end == $m) selected @endif>{{$m}}</option>
                                @endforeach
                            </select>
                            @error('end')
                                <p class="text-danger">{{$message}}</p>
